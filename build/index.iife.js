var stylish = (function (exports) {
  ...
  const addToScope = cssObj => {
    ...
    if (!StylishContainer.scopes.includes(cssObj.scope)) {
      ...
      throw Error(`ERROR: class "${cssObj.class}" already exists in scope "${cssObj.scope}"`);
    }
    return cssObj;
  };
  ...
  const addToClasses = cssObj => {
    ...
    StylishContainer.sheets.find(sheet => sheet.id === MAIN_SHEET_ID).sheet.insertRule(`.${cssObj.scope}__${cssObj.class} { ${cssObj.rules.join(";")} }`);
    return cssObj;
  };
  ...
  const updateMediaQueries = cssObj => {
    ...
    Object.keys(cssObj.media).forEach(mediaQuery => {
      ...
      mediaSheet = StylishContainer.sheets.find(sheet => sheet.sheet.media["mediaText"].toLowerCase().replace(/\s/g, "") === mediaQuery.toLowerCase().replace(/\s/g, ""));
      ...
      mediaSheet.sheet.insertRule(`.${cssObj.scope}__${cssObj.class} { ${cssObj.media[mediaQuery].join(";")} }`);
    });
  };

  const pushCssObj = cssObj => flow_1(addToScope, addToClasses, updateMediaQueries)(cssObj);
  ...
  var index = { CSSObject, addToScope, updateMediaQueries };

  exports.CSSObject = CSSObject;
  exports.addToScope = addToScope;
  exports.updateMediaQueries = updateMediaQueries;
